//! The archive module contains structs for interacting with a Fonds archive
...
///
/// # Errors
/// Error if the path doesn't exist or isn't inside a Fonds archive.
pub fn find_archive_path(path: &Path) -> anyhow::Result<PathBuf> {
    ...
    anyhow::bail!(format!(
        "{} is not inside a Fonds Archive. Run `taf conf init` to create an archive at this location.",
        abs_path.to_string_lossy()
    ))
}

/// Get the qualified name as parts of a Fonds from the {org}/{name} format.
/// # Errors
...
mod test {
    ...
    fn get_name_parts_when_qualified_name_correct_expect_name_parts() {
        ...
        let actual = cut("fonds/test").unwrap();
        let expected = ("fonds".to_owned(), "test".to_owned());
