//! Taf Server-specific errors

use actix_web::{error, http::StatusCode, HttpResponse};
use derive_more::{Display, Error};
use std::io;

/// Collection of possible HTTP errors
#[derive(Debug, Display, Error)]
pub enum HTTPError {
    #[display("404 Not Found")]
    /// 404
    NotFound,
    #[display("Unexpected server error")]
    /// 500
    InternalServerError,
}

/// Collection of possible CLI errors
#[derive(Debug, Display, Error)]
pub enum CliError {
    /// Database connection error
    #[display("Failed to connect to the database")]
    DatabaseConnectionError,
    /// A generic fallback error
    #[display("A CLI error occurred")]
    GenericError,
    /// Errors during archive parsing
    #[display("Failed to parse the archive ")]
    ArchiveParseError,
    /// The archive failed one or more `check` validations
    #[display("Archive failed `taf-server check` validation")]
    CheckFailed,
}

impl From<io::Error> for CliError {
    fn from(_error: io::Error) -> Self {
        Self::GenericError
    }
}

/// Collection of possible taf-server web-facing errors
#[derive(Debug, Display, Error)]
pub enum TafServerError {
    /// Errors generated by the Git server
    #[display("A Git server occurred")]
    GitError,
}

#[expect(clippy::missing_trait_methods, reason = "Use implicit implementation")]
impl error::ResponseError for TafServerError {
    fn error_response(&self) -> HttpResponse {
        HttpResponse::build(self.status_code()).body(self.to_string())
    }

    fn status_code(&self) -> StatusCode {
        match *self {
            Self::GitError => StatusCode::INTERNAL_SERVER_ERROR,
        }
    }
}
