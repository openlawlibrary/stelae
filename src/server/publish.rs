...
pub async fn serve_archive(
    ...
    let archive = Archive::parse(archive_path, PathBuf::from(raw_archive_path)).unwrap();
    dbg!(&archive);
    //     let root_stele = archive.get_root();
    ...
    //}
    {
        ...
        cfg.service(
            web::scope("/us/ca/cities/san-mateo")
                ...
                .route("/{pdfs:.*/.*pdf}", web::get().to(index)), // .service(index)
                                                                  // .service(test),
        );
    }
    {
